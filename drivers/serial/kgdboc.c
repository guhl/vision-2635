...
static int config_retry_time = 5;
...
static int configure_kgdboc(void)
{
	...
	printk("kgdb configure_kgdboc cptr = %s\n", cptr);
...
do_register:
	...
	configured = 1;
	printk("kgdb configure_kgdboc configured = %d!\n", configured);

	return 0;

noconfig:
	printk("kgdb configure_kgdboc reached noconfig!\n");
	config[0] = 0;
	configured = 0;
	cleanup_kgdboc();
